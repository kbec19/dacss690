...
## Analyzing Experimental Data Based on Brader et al 2008 AJPS Article 
...
Note: Data for this example is built into R 

In this example, we will be analyzing experimental data from a 2008 publication by Brader et al, "What Triggers Public Opposition to Immigration" examining how different primes influence white American's support for immigration. 

This was a 2X2 experimental design where authors created a fake newspaper story that manipulated two factors:
1. If story was about a Latino or European immigrant - variable 'eth' in data 
2. If story had a positive or negative tone about immigration - variable 'tone' in data

```{r framing}

#Start by loading the data - note you must have the mediation package installed and loaded for this to run 
...
First time you get any data, you should always investigate to understand its structure and format. Always think about how your data is formatted - i.e. is it a factor, number, character, etc.? In R, and other statistical software packages, the variable type determines how you can analyze and report that piece of data. 

The below code will help you quickly assess your data structure to understand what variables are numerical, factor, character, etc.

```{r }
names(framing)
#Variable Review Code
skim(framing) #Use this to quickly get unweighted summary values for your numeric variables
freq(framing$educ) #Use this to get the frequency of values within a variable 
attributes(framing$educ) #Use this to get the value levels for a variable 
class(framing$cond) #Use this to see what type of variable you have 
freq(framing$age)
summary(framing$age) #Use this for a continuous variable 
...
```{r}
#It can be important to transform your data to make analysis and reporting easier  
...
framing$tone <-as.factor(framing$tone) #Makes variable a factor for use in a chi-square balance test calculation
...
framing$treat_a<-as.factor(framing$treat) #Makes variable a factor for use in a chi-square balance test calculation
...
framing$cond2 <- relevel(framing$cond2, ref = 3)  # This line of code changes the reference group for analysis we will conduct shortly 
